...
class ItemAutocompleteField extends ConsumerStatefulWidget {
  const ItemAutocompleteField({super.key, required this.listId});
...
class _ItemAutocompleteFieldState
  ...
  Future<void> _submit({CatalogItem? catalogItem}) async {
    final text = catalogItem?.displayName ?? _controller.text.trim();
    if (text.isEmpty) return;

    final repo = ref.read(listRepositoryProvider);
    if (catalogItem != null) {
      await repo.addItemFromCatalog(
        listId: widget.listId,
        catalogItem: catalogItem,
      );
    } else {
      await repo.addItem(listId: widget.listId, displayName: text);
    }
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    subtitle: Text(
                      item.isUserAdded ? 'Custom item' : 'From catalog',
                      style: theme.textTheme.bodySmall,
                    ),
                    onTap: () => _submit(catalogItem: item),
                  );
                },
              ),
            ),
